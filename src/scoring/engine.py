...
def _find_matches(text_items: list[str], keywords: list[str], field_name: str) -> list[dict]:
    """
    Find which text items contain any of the keywords.
    Returns citations with source field attribution.
    """
    citations = []
...
def _find_specialty_matches(specialties_raw: str, target_specialties: list[str]) -> list[dict]:
...
def _has_quantitative_claim(citations: list[dict]) -> bool:
...
def score_facility(capability: str, texts: dict) -> dict:
    ...
    Args:
        capability: One of the keys in CAPABILITY_KEYWORDS
    ...
    Returns:
        {
            ...
            "evidence_citations": [{"field": "capability", "text": "..."}, ...],
            "fields_matched": [...],
            "match_count": int
        }
    """
...
def score_facility_all_capabilities(texts: dict) -> dict[str, dict]:
    """
    Score a facility across all capabilities at once.
